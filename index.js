...
const regMail = (recipientEmail) => {
  `from: support@greenstockscapital.com,
  to: ${recipientEmail},
  subject: 'Welcome to the Team!',
  text: We are glad to have you on board!,
  html: <p>Use the following link to login:</p><p><a href="https://greenstockscapital.com/login">Start Investing</a></p>`;
};
...
const sendEmail = async (recipientEmail) => {
  ...
  // Send the email
  try {
    const info = await transporter.sendMail(regMail(recipientEmail));
...
app.post('/login', async (req, res) => {
  try {
    ...
    if (!isPasswordValid) {require('dotenv').config();
    const express = require('express');
    const app = express();
    // const { MailtrapClient } = require('mailtrap');
    const bcrypt = require('bcryptjs');
    ...
    app.use(cors(
      {
        origin: 'https://greenstockscapital-87176639ed4e.herokuapp.com',
        credentials: true
      }
    ));
